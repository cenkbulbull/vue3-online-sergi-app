				...
				<div class="card-content white-text">
					...
					<ul class="collection">
						<li class="collection-item black-text" v-for="mesaj in mesajlar" :key="mesaj">
							<div>
								{{mesaj.icerik}} <a href="#" class="secondary-content"><i class="fas fa-trash red-text" @click="mesajSil(mesaj.id)"></i></a>
							</div>
						</li>
					</ul>
				</div>
		...
		setup(){
			...
			const mesajGonder = async()=>{
				if (mesajText.value!="") {
					...
					await firestoreRef.collection("mesajlar").add(yeniMesaj)
					.then(()=>{
						mesajText.value=""
					})
				}
